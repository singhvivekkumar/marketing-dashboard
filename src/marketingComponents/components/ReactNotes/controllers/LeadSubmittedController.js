...
export const UpdateLeadSubmitted = (req, res) => {
  ...
  console.log("queryData for UpdateLeadSubmitted", queryData);

  LeadSubmittedModel.update(queryData, {
    ...
    .then((num) => {
      if (num == 1) {
        ...
        res.status(200).json({
          success: true,
          data: num,
          message: "Lead Submitted updated successfully",
        });
      } else {
        console.log("Updated  UnSuccessfully");
        // console.log("bhgh3ew3433333333333",res.status)
        res.send({
          message: `Cannot update UpdateLeadSubmitted with id=${id}. was not found or req.body is empty!`,
        });
      }
    })
    .catch((err) => {
...
export const DeleteLeadSubmitted= (req, res) => {
  ...
  LeadSubmittedModel.destroy({
    where: { id: queryData["id"] },
  })
    .then((num) => {
      if (num == 1) {
        console.log("DELETE Successfully");
        res.status(200).json({ message: " deleted Successfully for Lead Submittted" });
      } else 
      {
        
        res.send({
          message: `Cannot  Delete Lead Submittted with id=${id}. was not found or req.body is empty!`,
        });
    ...
    .catch((err) => {
      ...
      res.status(500).send({
        message: "Error DELETE Lead Submittted with id=" + id,
      });
    });
