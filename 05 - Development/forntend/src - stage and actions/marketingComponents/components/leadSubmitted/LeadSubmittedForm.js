...
const LeadSubmittedForm = () => {
  ...
  const [dynamicOptions, setDynamicOptions] = useState({
    tenderType: ["SL", "LT", "MT"],
    presentStatus: ["Won", "Lost", "Submitted", "In Progress"],
  });
  ...
  useEffect(() => {
    axios
      ...
      .then(function (response) {
        ...
        axios
          .get(nodeIp + "/getLeadSubmittedFilters")
          .then((res) => {
            if (res.data.success) {
              console.log("Defence Filter:", res.data.data.status);
              setDynamicOptions({
                ...
                presentStatus:
                  res.data.data.status.length > 0
                    ...
                    : ["Won", "Lost", "Submitted", "In Progress"],
              });
            }
          })
  ...
  const handleAutofill = async (refNo) => {
    if (!refNo) {
      alert("Please enter a Lead Reference Number first.");
      return;
    }

    try {
      // 1. Fetch data from backend using the refNo
      const response = await axios.get(
        `${ServerIp}/getDomesticLeadRefNo/${refNo}`
      );

      if (response.data.success) {
        const dlData = response.data.data;

        // 2. Map Domestic Lead fields -> Lead Submitted fields
        // Use the reset() function from react-hook-form to update multiple fields at once
        reset({
          ...control._formValues, // Keep any data already typed in other fields
          tenderReferenceNo: dlData.leadReferenceNo, // The link
          tenderName: dlData.tenderName,
          customerName: dlData.customerName,
          customerAddress: dlData.customerAddress,
          tenderType: dlData.tenderType,
          bidOwner: dlData.leadOwner, // Mapping Lead Owner to Bid Owner
          tenderDate: dlData.tenderDated
            ? dlData.tenderDated.split("T")[0]
            : "", // Format for date input
          rfpDueDate: dlData.lastDateOfSub
            ? dlData.lastDateOfSub.split("T")[0]
            : "",
          // Add any other matching fields here
        });

        alert("✨ Form autofilled successfully from Domestic Lead!");
      }
    } catch (error) {
      console.error("Autofill error:", error);
      alert("Could not find a Lead with that Reference Number.");
    }
  };
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                >
                  ...
                  <Grid container spacing={3}>
                    ...
                    {/* Tender Reference No / Lead ID with Autofill */}
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <TextField
                            {...field}
                            fullWidth
                            label="Lead Reference No (BQ ID)"
                            placeholder="Type BQ Ref No and click Search"
                            error={!!errors.tenderReferenceNo}
                            helperText={
                              errors.tenderReferenceNo
                                ? errors.tenderReferenceNo.message
                                : "Click the icon to autofill from BQ"
                            }
                            InputProps={{
                              endAdornment: (
                    ...
                    <Grid item xs={12} sm={6} md={4}>
                      ...
                      >
                        <Controller
                          ...
                          render={({ field }) => (
                            <TextField
                              ...
                              fullWidth
                              error={!!errors.leadReferenceNo}
                              helperText={errors.leadReferenceNo?.message}
                              sx={{
                                "& .MuiOutlinedInput-root": { borderRadius: 3 },
                              }}
                            />
                        ...
                        <Button
                          ...
                          sx={{ mt: 1, borderRadius: 2, height: 40 }}
                        >
                          Fetch
                        </Button>
                      </Box>
                    ...
                    <Grid item xs={12} sm={6} md={4}>
                      <Controller
                        ...
                        render={({ field }) => (
                          ...
                          >
                            {dynamicOptions.presentStatus.map((status) => (
...
function ViewLeadSubmittedData(props) {
  ...
  const handleDeleteRow = async (id) => {
    ...
    console.log(
      "api for delete in lead submitted : ",
      `${ServerIp}/getLeadSubmitted`
    );
              ...
              >
                {[
                  ...
                  { label: "RFP Due Date", key: "rfpDueDate" },
                  {
                    ...
                    key: "dmktgInPrincipalApprovalRxdOn",
                  },
                  {
                    label: "Selling In Principal Approval Rxd On",
                    key: "sellingPriceApprovalInitiatedOn",
                  },
                  { label: "Bid Submitted On", key: "bidSubmittedOn" },
                  {
                    label: "Approval SBU  Finance On",
                    key: "approvalSBUFinanceOn",
                  },
                  { label: "Approval GM On", key: "approvalGMOn" },
                  {
                    label: "Sent to Finance GM Dmktg Approval Rxd On",
                    key: "sentToFinanceGMDmktgApprovalRxdOn",
                  },
...
function ExcelUploadAndValidate({ user, ServerIp }) {
  ...
  const handleDownloadSampleExcel = () => {
    const fileUrl = "/sample/Lead_Submitted_Sample.xlsx"; // fixed public path
    const link = document.createElement("a");
    link.href = fileUrl;
    link.download = "Lead_Submitted_Sample.xlsx";
