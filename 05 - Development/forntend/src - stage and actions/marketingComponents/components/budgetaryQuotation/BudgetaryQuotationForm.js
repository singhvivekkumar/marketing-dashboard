...
const BudgetaryQuotationForm = () => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then(function (response) {
        ...
        console.log(nodeIp + API);
        axios.get(nodeIp + API).then((response) => {
          console.log(" table ddd data : ", nodeIp + API, response.data);
          setOrderData(response.data);
        });
        ...
        axios.get(nodeIp + "/getAllLeadReferences").then((res) => {
          if (res.data.success) {
            setLeadOptions(res.data.data);
          }
        });

        axios
          .get(nodeIp + "/getBudgetaryQuotationFilters")
          .then((res) => {
            if (res.data.success) {
              console.log("Defence Filter:", res.data.data);
              setDynamicOptions({
                ...
                presentStatus:
                  res.data.data.status.length > 0
                    ...
                    : ["Budgetary Quotation Submitted", "Not Participated"],
                segment: ["sonar"],
                project: ["ACS"],
              });
            }
      ...
      .catch(function (error) {
        console.log("error while hitting bq api data : ", error);
        SetServerIp("172.195.120.135");
      });
  }, [submitSuccess]);
  ...
  const handleLeadSelect = (refNo) => {
    const lead = leadOptions.find((l) => l.leadReferenceNo === refNo);

    if (!lead) return;

    // ✅ Normalize Defence Values (IMPORTANT FIX)
    const normalizeDefence = (val) => {
      if (!val) return "";
      const lower = val.toLowerCase();

      if (lower.includes("defence")) return "Defence";
      if (lower.includes("non")) return "Civil";

      return val;
    };

    // ✅ Track autofilled fields for UI highlight
    const fields = [
      "bqTitle",
      "customerName",
      "customerAddress",
    ...
    // ✅ Auto add missing dropdown values

    if (lead.segment && !dynamicOptions.segment.includes(lead.segment)) {
      setDynamicOptions((prev) => ({
        ...prev,
        segment: [...prev.segment, lead.segment],
      }));
    }

    if (lead.project && !dynamicOptions.project.includes(lead.project)) {
      setDynamicOptions((prev) => ({
        ...prev,
        project: [...prev.project, lead.project],
      }));
    }

    if (
      lead.defenceOrNonDefence &&
      !dynamicOptions.defenceAndNonDefence.includes(lead.defenceOrNonDefence)
    ) {
      setDynamicOptions((prev) => ({
        ...prev,
        defenceAndNonDefence: [
          ...prev.defenceAndNonDefence,
          lead.defenceOrNonDefence,
        ],
      }));
    }

    // ✅ Reset form with FULL mapping
    reset({
      leadReferenceNo: lead?.leadReferenceNo || "",

      // TEXT FIELDS
      bqTitle: lead?.tenderName || "",
      customerName: lead?.customerName || "",
      customerAddress: lead?.customerAddress || "",
      leadOwner: lead?.leadOwner || "",

      // DROPDOWNS
      defenceAndNonDefence: normalizeDefence(lead?.defenceOrNonDefence),
      segment: lead?.segment || "",
      project: lead?.project || "",

      // NUMERIC FIELDS (convert to string for React)
      estimateValueInCrWithoutGST:
        lead?.estimateValueInCrWithoutGST !== undefined &&
        lead?.estimateValueInCrWithoutGST !== null
          ? String(lead.estimateValueInCrWithoutGST)
          : "",

      submittedValueInCrWithoutGST:
        lead?.submittedValueInCrWithoutGST !== undefined &&
        lead?.submittedValueInCrWithoutGST !== null
          ? String(lead.submittedValueInCrWithoutGST)
          : "",
  ...
  render={({ field }) => (
    <TextField
      ...
      select
      fullWidth
      label="Lead Reference No"
      onChange={(e) => {
        field.onChange(e);
        handleLeadSelect(e.target.value);
      }}
    >
      {leadOptions.map((lead, index) => (
        <MenuItem key={index} value={lead.leadReferenceNo}>
          {lead.leadReferenceNo} ({lead.source})
        </MenuItem>
      ))}
    </TextField>
  )}
              ...
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                >
                  ...
                  <Autocomplete
                    ...
                    onChange={(e, value) => {
                      if (value) {
                        handleLeadSelect(value.leadReferenceNo);
                      }
                    }}
                    renderInput={(params) => (
                      <TextField
                        {...params}
                        label="Search Lead (Ref No / Title / Customer / Owner)"
                        variant="outlined"
                        sx={{
                          backgroundColor: "#fff",
                          borderRadius: 1,
                      ...
                      >
                        <Box>
                          <Typography fontWeight={600}>
                            {option?.tenderName || "No Title"}
                          </Typography>

                          <Typography variant="body2" color="text.secondary">
                            {option?.customerName || "No Customer"} •{" "}
                            {option?.leadOwner || "No Owner"}
                          </Typography>
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <TextField
                            ...
                            label="Defence / Civil"
                            sx={{
                              backgroundColor: autoFilledFields.includes(
                                "defenceAndNonDefence",
                              )
                                ? "#eef3f7"
                                : "white",
                    ...
                    <Grid item xs={12} sm={4}>
                      <Controller
                        ...
                        render={({ field }) => (
                          ...
                          >
                            {dynamicOptions.presentStatus.map((s) => (
                    ...
                    <Grid item xs={12} sm={4}>
                      <Controller
                        ...
                        render={({ field }) => (
                          ...
                          >
                            {dynamicOptions.segment.map((opt) => (
                              <MenuItem key={opt} value={opt}>
                                {opt}
                              </MenuItem>
                            ))}
                    ...
                    <Grid item xs={12} sm={4}>
                      <Controller
                        ...
                        render={({ field }) => (
                          ...
                          >
                            {dynamicOptions.project.map((opt) => (
                              <MenuItem key={opt} value={opt}>
                                {opt}
                              </MenuItem>
                            ))}
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <TextField
                            ...
                            multiline
                            rows={1}
                            helperText="remarks competitors with commas"
                            sx={{
                              "& .MuiOutlinedInput-root": {
                                borderRadius: 3,
                              },
                            ...
                            <Box sx={{ flex: 1, minWidth: 0 }}>
                              ...
                              >
                                {uploadedDocument
                                  ...
                                  : `${(documentFile.size / 1024).toFixed(
                                      1,
                                    )} KB`}
                              </Typography>
                            </Box>
    ...
    axios
      ...
      .then((response) => {
        ...
        const pdfBlob = new Blob([response.data], {
          type: "application/pdf" || "application/doc",
        });
    ...
    tableData
      .filter((row) => {
        ...
        const matchesTenderType =
          defenceFilter === "all" ||
          row.defenceAndNonDefence?.toLowerCase() ===
            defenceFilter.toLowerCase();
