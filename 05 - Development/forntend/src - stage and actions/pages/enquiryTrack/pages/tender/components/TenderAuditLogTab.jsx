...
export default function TenderAuditLogTab({ tender }) {
  return (
    <Box>
      {tender.auditLog.map((entry, i) => (
        <React.Fragment key={i}>
          <Box sx={{ display: 'flex', gap: 2, py: 1.5, alignItems: 'flex-start' }}>
            <Avatar sx={{ width: 28, height: 28, fontSize: '0.65rem', fontWeight: 700, backgroundColor: '#e6f6f6', color: '#0e7c7b', flexShrink: 0, mt: 0.3 }}>
          ...
          {i < tender.auditLog.length - 1 && <Divider />}
        </React.Fragment>
      ))}
    </Box>
  )
