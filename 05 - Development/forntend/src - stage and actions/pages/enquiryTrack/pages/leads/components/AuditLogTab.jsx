...
export default function AuditLogTab({ lead }) {
  return (
    <Box>
          ...
          <Box sx={{ display: 'flex', gap: 2, py: 1.5, alignItems: 'flex-start' }}>
            ...
            <Box sx={{ flex: 1 }}>
              <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1, mb: 0.3 }}>
                <Typography variant="caption" sx={{ fontWeight: 600, color: '#1a2236' }}>
                  {entry.user}
                </Typography>
                <Typography sx={{ fontFamily: "'JetBrains Mono'", fontSize: '0.65rem', color: '#97a3b5' }}>
                  {entry.timestamp}
                </Typography>
              </Box>
              <Typography variant="body2" sx={{ color: '#5a6880', lineHeight: 1.6 }}>
                {entry.action}
              </Typography>
            </Box>
          </Box>
          {i < lead.auditLog.length - 1 && <Divider />}
        </React.Fragment>
      ))}
    </Box>
  )
